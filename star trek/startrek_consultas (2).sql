...
espanha             4 
 ...
 select nacionalidade, count(*) from actores group by nacionalidade;
...
cabo                5 
 ...
 select graduacion, count(*) from personaxes group by graduacion;
...
voyager 
 ...
 select distinct nomen from naves,visitas where cods='s1' and naves.codn=visitas.codn;
...
select nomper from personaxes where codper not in (select distinct codper from interpretespel);
...
select count(*) from interpretesser where coda ='a18';
...
select actores.coda, count(interpretespel.coda) from actores left join interpretespel on actores.coda=interpretespel.coda group by actores.coda;
...
select noma from actores, interpretesser where actores.coda=interpretesser.coda group by noma,cods having count(interpretesser.codper)>1;
...
select distinct  nomper from personaxes, interpretespel where personaxes.codper=interpretespel.codper group by nomper,interpretespel.coda having count(interpretespel.coda)>1;
...
select nomper from personaxes, interpretespel where personaxes.codper=interpretespel.codper group by nomper having count(interpretespel.coda)>1;
...
select noma from actores, personaxes, interpretesser where actores.coda=interpretesser.coda and interpretesser.codper=personaxes.codper and noma like '%a%' and nomper like '%a%';
